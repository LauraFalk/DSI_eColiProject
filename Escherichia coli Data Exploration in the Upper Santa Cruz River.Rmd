...
Triple hashes (###) are meant to indicate code that has been commented-out or added in order to expedite the processing (by use of a saved csv file of processed data rather than repeating the processing each time). If code is fully automated, these chunks are to be altered.
```{r}
...
#LAURA This still isn't giving the attributes that I want.  I need to pull out the between median values so that I can attribute them as "stable"

To determine if the river is rising, I'm going to sum the difference of the last 24 readings?
How about 12? I don't want to associate too much with temperature. Can use corr to determine if this is reasonable.
```{r}
CFS_Med <-  median(USGS_Standardized$Discharge_CFS, na.rm = TRUE)
CFS_Med_low <- median(USGS_Standardized$Discharge_CFS, na.rm = TRUE) - 5
CFS_Med_High <- median(USGS_Standardized$Discharge_CFS, na.rm = TRUE) + 5
0
# If the reading is less than the lowest median, 0 score for falling 
USGS_Standardized$Medlow <- ifelse(USGS_Standardized$Discharge_CFS<CFS_Med_low,0,1)

# If the reading is greater than the higher median bracket, 1 score for rising
USGS_Standardized$Medhigh <- ifelse(USGS_Standardized$Discharge_CFS>CFS_Med_High,1,0)
...
USGS_Standardized$MedDif2 <- ifelse(USGS_Standardized$DisDif>0,1,0)

# Summary statistic of all three things
USGS_Standardized$MedSum <- USGS_Standardized$Medlow + USGS_Standardized$Medhigh + USGS_Standardized$DisDif2


#USGS_Standardized$Stage24 <- rollsumr(USGS_Standardized$DisDif2, k = 96, fill = NA)

USGS_Standardized$Stage24b <- ifelse(USGS_Standardized$MedSum <=1, "Falling", 
          ifelse(USGS_Standardized$MedSum > 1 & USGS_Standardized$MedSum <= 2, "stable",
          ifelse (USGS_Standardized$MedSum == 3,"Rising", NA)))
USGS_Standardized %>%
  count(Stage24b == "Falling")

```

```{r}
USGS_Standardized %>%
 filter(DateTime2>as.POSIXlt.character("2015-10-01") 
        & DateTime2 < as.POSIXlt.character("2015-10-15")) %>%
  filter(Discharge_CFS<100) %>%
  ggplot(aes(x=DateTime2, y=Discharge_CFS, color=Stage24b))+
  geom_point()
```
...
data_Merge3 <- data_Merge2 %>%
  select(ActivityConductingOrganizationText, MonitoringLocationIdentifier, QuantificationLimit, DailyFlowCount, DateTime2, StandardizedResult, Discharge_CFS, USGS_Staff, QuantificationLimit, Precipitation..in., Tmax..F., Tmin..F., Previous3Precip, Previous7Precip, Previous30Precip, Season, NinXTS, TOD)
...
I want to look at nulls from the climate data. I think this will have an effect on my corrplot. With removed rows, I still have 896 to play with.
```{r}
...
1. Previous 30 Precip - interesting that it has more of an effect than 7 an 3. Possibly something to do with flooding washing out? Or interactions when the water is extremely low and it hasn't rained in months?
2. T min
