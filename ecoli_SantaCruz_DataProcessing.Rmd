...
https://origin.cpc.ncep.noaa.gov/products/analysis_monitoring/ensostuff/ONI_v5.php) - El Nino / La Nina
...
DailyClimate$PreviousTmin <- lag(DailyClimate$Tmin..F.,2)
# Use the previous day's Tmin due to data posting interval. This may be used for future forecasting.
...
data_Merge2 %>%
  ...
  geom_hline(yintercept=575, linetype="dashed", 
                color = "dark green", size=0.2)
```
This creates a rolling minimum value at different intervals. As precipitation data is currently questionable, I would like to shift to using minimum temperature instead (11/4/2022). According to correlation plots run previously, tMin is nearly as correlated as previous 30 precip.

```{r}
myMin <- function(x) ifelse( !all(is.na(x)), min(x, na.rm=T), NA)

data_Merge2 <- data_Merge2 %>%
  mutate(ThreeDayMin = slide_dbl(Tmin..F., myMin, .before = 2, .complete = FALSE))

data_Merge2 <- data_Merge2 %>%
  mutate(TenDayMin = slide_dbl(Tmin..F., myMin, .before = 9, .complete = TRUE))

data_Merge2 <- data_Merge2 %>%
  mutate(ThirtyDayMin = slide_dbl(Tmin..F., myMin, .before = 29, .complete = TRUE))

```

```{r}
data_Merge2 %>%
  filter(!is.nan(ThreeDayMin)) %>%
  ggplot(aes(x=ThreeDayMin, y=StandardizedResult)) +
  geom_point() +
  geom_hline(yintercept=126, linetype="dashed", 
                color = "blue", size=0.2) +
  geom_hline(yintercept=235, linetype="dashed", 
...
data_Merge2 %>%
  filter(!is.nan(TenDayMin)) %>%
  ggplot(aes(x=TenDayMin, y=StandardizedResult)) +
  geom_point() +
  geom_hline(yintercept=126, linetype="dashed", 
                color = "blue", size=0.2) +
  geom_hline(yintercept=235, linetype="dashed", 
...
data_Merge2 %>%
  filter(!is.nan(ThirtyDayMin)) %>%
  ggplot(aes(x=ThirtyDayMin, y=StandardizedResult)) +
  geom_point() +
  geom_hline(yintercept=126, linetype="dashed", 
                color = "blue", size=0.2) +
  geom_hline(yintercept=235, linetype="dashed", 
...
traindata_numeric <- traindata %>%
  select(c("StandardizedResult", "ecoli_235","ecoli_575", "Discharge_CFS", "USGS_Staff", "Precipitation..in.", "PreviousTmax", "PreviousTmin", "Previous3Precip", "Previous7Precip", "Previous30Precip", "NinXTS", "TOD", "Stage", "SeasonNumeric", "ACO_Numeric", "DistFromSonoita", "ThreeDayMin", "TenDayMin", "ThirtyDayMin"))
...
cor2 <- cor(traindata %>%
  select(c("ecoli_575", "Previous30Precip", "PreviousTmin", "Discharge_CFS", "Stage","NinXTS", "TOD","ACO_Numeric", "DistFromSonoita","ThreeDayMin", "TenDayMin", "ThirtyDayMin")))
...
cor3<- cor(traindata %>%
  select(c("ecoli_235", "Previous30Precip", "PreviousTmin", "Discharge_CFS", "Stage","NinXTS", "TOD","ACO_Numeric", "DistFromSonoita","ThreeDayMin", "TenDayMin", "ThirtyDayMin")))
...
write.csv(data_Merge3 %>%
            select("PreviousTmin", "Discharge_CFS", "Stage","NinXTS", "TOD", "DistFromSonoita","ecoli_235", "ecoli_575" ), "Data/Processed/ecoli_attributed2.csv")
```
