...
# ecoli_235 <- ecoli_attr[,2:7]
ecoli_575 <- ecoli_attr %>%
  select (-ecoli_235, -X)
...
final575model <- xgboost(data = xgb_train, max.depth = 5, nrounds = 20, verbose = 0)

set.seed(0)
# Predict the new values
pred <- predict(final575model, as.matrix(test_x))
...
pred <- predict(final575model, as.matrix(test_x3))
pred <-  as.numeric(pred > 0.5)
confusionMatrix(factor(pred),factor(test_y))
```
...
pred <- predict(final575model, as.matrix(test_x4))
pred <-  as.numeric(pred > 0.5)
confusionMatrix(factor(pred),factor(test_y))
...
xgb.save(final575model, "Data/Processed/XGBmodel575")
